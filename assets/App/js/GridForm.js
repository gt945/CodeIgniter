...
//                         hash[i]=xui.Date.format(o, "yyyy-mm-dd hh:nn:ss");
				...
				if(recordIds.length>0){
					...
					AJAX.callService(ns.getProperties("gridName"),"set",rqsD,function(rsp){
						if(rsp.data==1){
							xui.message("保存成功!");
							ns.fireEvent("afterUpdated", [recordIds, db.getDirtied(true)], ns);
							db.updateValue();
							ns.setDirty(false);
...
//                     dataFromUI[i]=xui.Date.format(o, "yyyy-mm-dd hh:nn:ss");
		...
		_lock_onchange:function(profile,oldValue,newValue,force,tag){
			...
			var alias=profile.alias.replace(/^lock\_/, "");
			ns[alias].setDisabled(!newValue);
			if(newValue){
				uictrl.setImage("@xui_ini.appPath@image/unlock.png");
			}else{
				uictrl.setImage("@xui_ini.appPath@image/lock.png");
				ns[alias].updateValue().refresh();
			}
		}
	}
});
