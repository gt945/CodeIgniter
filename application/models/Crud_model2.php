...
class Crud_model2 extends CI_Model {
	...
	const CRUD_TABLE = 'crud_table2';
	const CRUD_FIELD = 'crud_field2';
	...
	public function install($table) {
		...
		if (count ( $fields ) > 0) {
			...
			$tid = $this->db->get ( Crud_model2::CRUD_TABLE )->row ( 'id' );
			if (! $tid) {
				$data = array (
						'name' => $table,
						'caption' => $table
				...
				$this->db->insert ( Crud_model2::CRUD_TABLE, $data );
				$tid = $this->db->insert_id ();
			}
			if ($tid) {
				$this->db->select ( 'id,name' );
				$this->db->where ( 'tid', $tid );						//get all fields in crud_field table
				$exist_fields = $this->db->get ( Crud_model2::CRUD_FIELD )->result_array ();
				...
				foreach ( $fields as $f ) {
					...
					if (! $find) {										//append to crud_field
						...
						switch ($f->type) {
							case "int":
								$data['type'] = Crud_model2::TYPE_NUMBER;
								break;
							case "varchar":
								$data['type'] = Crud_model2::TYPE_LABEL;
								break;
							case "datetime":
								$data['type'] = Crud_model2::TYPE_DATETIME;
								break;
							case "date":
								$data['type'] = Crud_model2::TYPE_DATE;
								break;
						}
						if ($f->name === 'id') {
							$data ['type'] = Crud_model2::TYPE_NUMBER;
							$data ['prop'] |= Crud_model2::PROP_FIELD_PRIMARY;
						}
	
						if ($f->name === 'uid') {
							$data ['type'] = Crud_model2::TYPE_SELECT;
						...
						if ($f->name === 'tree_code') {
							$data ['type'] = Crud_model2::TYPE_NUMBER;
							$data ['prop'] |= Crud_model2::PROP_FIELD_HIDDEN;
						}
						if ($f->name === 'pid') {
							$data ['type'] = Crud_model2::TYPE_SELECT;
						...
						if ($f->name === 'password') {
							$data ['type'] = Crud_model2::TYPE_PASSWORD;
						}
						$this->db->insert ( Crud_model2::CRUD_FIELD, $data );
					}
				}
				foreach ( $exist_fields as $k => $ef ) {				//delete not exist field
					$this->db->where ( 'id', $ef ['id'] );
					$this->db->delete ( Crud_model2::CRUD_FIELD );
				}
	...
	public function table($name) {
		...
		if ( ! $dbContext = $this->cache->get($cache_id)) {
			...
			$crud_field = array();
			$crud_table = $this->db->get ( Crud_model2::CRUD_TABLE, 1 )->row_array ();
			if ($crud_table) {
				...
				$crud_field = $this->db->get ( Crud_model2::CRUD_FIELD )->result_array ();
	...
	public function update($dbContext)
	{
		...
		$this->db->update ( Crud_model2::CRUD_TABLE, $save );
		$this->db->trans_start ();
		foreach ($dbContext->crud_field as $f) {
			...
			$this->db->update ( Crud_model2::CRUD_FIELD, $f );
		}
		$this->db->trans_complete ();
	}
	public function layout($table, $col = 1)
	{
		if ($col > 0 && $col < 5) {
			$dbContext = $this->table($table);
			$totalWidth = 15 * 2 + 10 * 2 + $col * 200 + ($col - 1) * 70;
			$totalHeight = ((int)(count($dbContext->crud_field) / $col) + 2) * 30 + 70;
			$dbContext->crud_table['x'] = $totalWidth;
			$dbContext->crud_table['y'] = $totalHeight;
			$index = 0;
			foreach($dbContext->crud_field as &$f) {
				$x = (int)($index % $col) + 1;
				$y = (int)($index / $col);
	...
	public function prepare(&$dbContext, $join = true)
	{
		...
		foreach ( $dbContext->crud_field as &$f ) {
			...
			if ($join) {
				if ($f ['type'] == Crud_model2::TYPE_SELECT
						|| $f ['type'] == Crud_model2::TYPE_MULTI
						|| $f ['type'] == Crud_model2::TYPE_BIT) {
		...
		switch ($oper) {
			...
			case 'create' :
				...
				foreach ( $dbContext->crud_field as $k => $f ) {
					...
					if (! isset ( $post [$k] )  || $f['prop'] & Crud_model2::PROP_FIELD_PRIMARY) {
						continue;
					}
					
					$data [$k] = $post [$k];
					if ($f ['type'] == Crud_model2::TYPE_PASSWORD && $data [$k] === "******") {
						unset ( $data [$k] );
						continue;
					}
					
// 					if ($f ['type'] == Crud_model2::TYPE_MULTI) {
...
// 						}
// 					}
// 					if ($f ['type'] == Crud_model2::TYPE_BIT) {
// 						$data_in = explode(',', $data [$k]);
// 						$data [$k] = 0;
// 						foreach ($data_in as $d) {
// 							if (isset($f ['_joined_data'][$d])) {
...
// 					}
					...
					if ($f ['type'] == Crud_model2::TYPE_SELECT) { 		// 如果为选择项，但是为-1，那么去掉
					...
					if ($f ['prop'] & Crud_model2::PROP_FIELD_UNIQUE) {
						$this->db->select("*");
	...
	public function get_left_join($table, $field, $field_info = null)
	{
		...
		switch ($field_info['type']) {
			case Crud_model2::TYPE_SELECT :
			case Crud_model2::TYPE_MULTI :
			case Crud_model2::TYPE_BIT :
...
// 				}
	...
	public function grid_header($dbContext, &$data)
	{
		...
		foreach ( $dbContext->crud_field as $k => $f ) {
			...
			switch ($f['type']) {
				case Crud_model2::TYPE_LABEL :
					$form_class = "xui.UI.ComboInput";
					$form_obj->properties->type = "none";
					break;
				case Crud_model2::TYPE_NUMBER :
					$form_class = "xui.UI.ComboInput";
					$form_obj->properties->type = "none";
					break;
					$form_obj->properties->type = "none";
				case Crud_model2::TYPE_DATE :
					$form_class = "xui.UI.ComboInput";
					$form_obj->properties->type = "date";
					break;
				case Crud_model2::TYPE_TIME :
					$form_class = "xui.UI.ComboInput";
					$form_obj->properties->type = "time";
					break;
				case Crud_model2::TYPE_DATETIME :
					$form_class = "xui.UI.ComboInput";
				...
				case Crud_model2::TYPE_MULTI :
					...
					break;
				case Crud_model2::TYPE_BOOL :
					$form_class = "xui.UI.ComboInput";
					$form_obj->properties->type = "bool";
					break;
				case Crud_model2::TYPE_TEXTAREA :
					$form_class = "xui.UI.ComboInput";
					$form_obj->properties->type = "none";
					$form_obj->properties->multiLines = true;
					break;
				case Crud_model2::TYPE_PASSWORD :
					$form_class = "xui.UI.ComboInput";
			...
			if ($f['prop'] & Crud_model2::PROP_FIELD_PRIMARY) {
				$form_obj->properties->readonly = true;
			}
			if ($f['prop'] & Crud_model2::PROP_FIELD_FILTER) {
				$setting->filter = true;
	...
	public function request_getlist($dbContext, $paras)
	{
		...
		if(isset($paras->sidx) && isset($paras->sord)) {
			...
			if (isset($dbContext->crud_field[$sort])) {
				if ($dbContext->crud_field[$sort]['type'] == Crud_model2::TYPE_SELECT) {
...
*/		
		...
		foreach ( $data as $d ) {
			...
			foreach ( $dbContext->crud_field as $k => $f ) {
				...
				} else if ($f ['type'] == Crud_model2::TYPE_PASSWORD) {
					array_push ( $tmp, "******" );
// 				} else if ($f ['type'] == Crud_model2::TYPE_MULTI && 0) {
// 					$new_val = array ();
...
// 				} else if ($f ['type'] == Crud_model2::TYPE_BIT && 0) {
// 					$new_val = array ();
// 					$select_data = $f['_joined_data'];
...
// 				array_push($tmp, false); //expanded
	...
	function request_tables($paras)
	{
		...
		$this->db->from(Crud_model2::CRUD_TABLE);
	...
	function request_fields($paras)
	{
		...
		$this->db->from(Crud_model2::CRUD_FIELD);
